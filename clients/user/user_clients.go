...
func NewUserClients(client config.IClientConfig) IUserClient {
...
func (u *UserClient) GetUserByToken(ctx context.Context) (*UserData, error) {
	...
	request := u.client.GetClient().Clone().
		...
		Get(fmt.Sprintf("%s/api/v1/auth/user", u.client.GetBaseURL()))
